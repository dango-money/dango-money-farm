...
const initWeb3Modal = async () => {
    ...
    if (provider.chainId !== window.connectedChain.hex && !window.location.href.includes(window.connectedChain.configUrl)) {
        window.location = window.connectedChain.configUrl;
    }
    ...
    provider.on("chainChanged",
        (chainId) => {
            if (chainId !== window.connectedChain.hex && !window.location.href.includes(window.connectedChain.configUrl)) {
                window.location = window.connectedChain.configUrl;
            }
        });
...
const getConnectedChain = (chainId) => {
